...
+ (XMPPIQ *)iqWithType:(NSString *)type to:(XMPPJID *)jid from:(XMPPJID *)jid1 elementID:(NSString *)eid child:(NSXMLElement *)childElement
{
    return [[XMPPIQ alloc] initWithType:type to:jid from:jid1 elementID:eid child:childElement];
}
...
//add by Wen Andy
- (id)initWithType:(NSString *)type to:(XMPPJID *)jid from:(XMPPJID *)jid1 elementID:(NSString *)eid child:(NSXMLElement *)childElement
{
    if ((self = [super initWithName:@"iq"]))
    {
        ...
        if (jid1)
            [self addAttributeWithName:@"from" stringValue:[jid1 full]];
        if (type)
